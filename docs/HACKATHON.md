...
- 2026-07-11 — Fixed the admin portal: it was gated only by the client access-key (`AdminGate`) and never held a Firebase `admin` claim, so in production every Firestore read (`accessRequests`, `users`) was denied and pending sign-in requests never appeared. Now `/admin` requires a signed-in admin **and** the access key (`RequireRole` wraps `AdminGate` in real mode; demo mode keeps key-only since its in-memory hooks bypass rules). Added the missing `accessRequests (status, createdAt)` composite index, surfaced load errors in the console instead of a false "no requests", and added `scripts/seedAdmin.ts` / `npm run seed:admin` to bootstrap the first admin (chicken-and-egg: `provisionUser` is itself admin-only). Deployed indexes + rules + hosting. **Remaining owner step:** run `npm run seed:admin` once with your chosen `ADMIN_EMAIL`/`ADMIN_PASSWORD` to create the admin login.
- 2026-07-11 — Full E2E verification of all four surfaces in demo mode (`npm run demo`) via a real browser, plus three read-only review agents (one per staff dashboard). Verified working: operator sign-up → pending screen → request appears in admin console → Approve promotes the account; admin provision form + staff list + access-key gate; ops dashboard (crowd/gates/transit/incidents/amenities all populated, incident resolve, Gemini situation brief, grounded ops chat); vendor dashboard (menu/stock, mark-sold-out, stall open/close, live order queue, advance-order); and the cross-role order loop (fan places order → appears in vendor queue → advance; sold-out flag propagates fan↔vendor). The review agents surfaced three **real-mode-only** defects (all now fixed + deployed): (a) missing `orders(stallId, createdAt)` composite index — vendor order queue never loaded in production; (b) missing `incidents(matchId, createdAt)` composite index — ops incident panel never loaded in production; (c) `approveAccessRequest` revoked refresh tokens on approval, which broke in-place promotion (forced a re-login). Fixes: added both indexes, removed the approval-time revoke (kept it for deactivation), and added a one-shot token refresh in `AuthProvider` so an approved user's elevated claim propagates live instead of stranding them on the fan surface.
