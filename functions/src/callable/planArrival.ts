 ...
 */
export const planArrival = onCall({ enforceAppCheck: true, memory: "512MiB" }, async (req) => {
  ...
  const startArea = typeof req.data?.startArea === "string" ? req.data.startArea.slice(0, 120) : undefined;
  ...
  const accessibilityNeeds = Array.isArray(req.data?.accessibilityNeeds)
    ? (req.data.accessibilityNeeds as unknown[]).filter((n): n is string => typeof n === "string").slice(0, 6)
    : [];
