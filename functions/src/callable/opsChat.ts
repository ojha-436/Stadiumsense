 ...
 */
export const opsChat = onCall({ enforceAppCheck: true, memory: "512MiB" }, async (req) => {
  ...
  const accessibility = count(
    fans.flatMap((f) => (Array.isArray(f.accessibilityNeeds) ? (f.accessibilityNeeds as string[]) : [])),
    (n) => n
  );
  ...
  const context: OpsChatContext = {
    ...
    sections: count(fans, (f) => String((f.seat as { section?: string })?.section ?? "unknown")).map(
      ([section, c]) => ({ section, count: c })
    ),
